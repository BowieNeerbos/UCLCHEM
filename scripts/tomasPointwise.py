...
allcrir,alluv=[0.1,1,10,100],[0.1,1,10,100] 
#crir=cosmic ray ion rate uv in Habing
notspecies=["Time","Density","gasTemp","av","zeta","point","BULK","SURFACE",
            "radfield"]
tomaslist=["SO","CH3OH","H2O","HCN","SO2"]
jonlist=["H2O","HCL","C3H2","CS","HS","H2CN","HCN","H2CS","CH3OH","NH3","NS",
         "H2S","CH","CN","HCO","O2","OH","OCS","HNO","HNC","CO2","SO","SO2"]
elementList = [
    "H",
    "D",
    "HE",
...
def enhanced(df,dens,vel,printit=False,shock="C",mylist=False): 
    #now i follow tomas' paper to define region boundaries
    """calculates enhancement factors and destruction factors
    of molecules
    """

    enhs,dests,maxs,starts,avgs1,avgs2,={},{},{},{},{},{} #lists of enhancement 
    #factors, destruction and max abundances respectively
    ...
    if mylist: #my cutoff values, at half max density
        ...
        # finds index where T is at half max, close to dissipation time
    ...
    for column in df:
        if jon:
            if column in jonlist:
                avg1=integrate.trapz(df[column][0:idx1],
                                     x=df["Time"][0:idx1])/df["Time"][idx1]
                #average abundance throughout the shock
                avg1=df[column][idx1]
                avg2=integrate.trapz(df[column][idx1:idx2],
                                     x=df["Time"][idx1:idx2])/(df["Time"][idx2]-df["Time"][idx1])
...
def JandCratio(species,mylist=False,save = False):
    ratio={}
    values=[]
    for dens in alldensity:
        ratio[dens]={}
        for vel in allv:
            ...
            df=uclchem.analysis.read_output_file(output_file=output_file)
            Cenhs,Cdests,Cmaxs,starts1,avgs1,avgs2,maxT=enhanced(df,dens,vel,shock="C",mylist=mylist)

            output_file="../output/shocks_output/{}shock{}-{}-1-1.dat".format("J",dens,vel)
            df=uclchem.analysis.read_output_file(output_file=output_file)
            Jenhs,Jdests,Jmaxs,starts2,avgs1,avgs2,maxT=enhanced(df,dens,vel,shock="J",mylist=mylist)
            ratio[dens][vel]=Jmaxs[species]/Cmaxs[species]
            values.append(Jmaxs[species]/Cmaxs[species])
            print( starts1[species]-starts2[species])
    #print(ratio)
    #print(max(values))
    f=plt.figure()
    f.set_figwidth(11)
    ...
    if mylist:
        ...
        name="tomas"
    if save:
        plt.savefig("../output/figures/{}Enh_{}.png".format(name,species))
    plt.show()
    plt.close()
...
enhanced(df,1e6,7,shock="J")
for molecule in tomaslist:
    JandCratio(molecule,mylist=False)
