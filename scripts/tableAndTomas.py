...
it also corresponds to the tomas figures
"""
import sys
import os
    ...
    for column in df:
        if jon:
            if column in jonlist:
                ...
                start=df[column][0]
                change=avg1/start
                changelong=avg3/start#was avg2/start for table
...
def JandCratio(species,mylist=True,save=False,longrun=False):
    ...
    for dens in alldensity:
        for vel in allv:
            plt.scatter(vel,np.log10(dens),c=ratio[dens][vel],
                        norm=clr.LogNorm(vmin=1E-4,vmax=1E4),
    ...
    if save:
        plt.tight_layout()
        plt.savefig("../output/J_C_ratios/{}{}Enh_{}.png".format(name,region,species))
    plt.show()
    plt.close()
...
for molecule in df:
    ...
    if molecule not in elementList and molecule not in notspecies:
        if not "@" in molecule and not "#" in molecule and not "+" in molecule:
            if not os.path.exists("../output/J_C_ratios/bowiemidEnh_{}.png".format(molecule)):
                JandCratio(molecule,mylist=True,longrun=False,save=True)
            else:
                print("skipped", molecule)
                #JandCratio(molecule,mylist=True,longrun=True,save=False)
print (i)
JandCratio("NH",longrun=True, save = False)