...
jonlist=["H2O","HCL","C3H2","CS","HS","H2CN","HCN","H2CS","CH3OH","NH3","NS","H2S","CH","CN","HCO","O2","OH","OCS","HNO","HNC","CO2","SO","SO2", "H3O+"]
elementList = [
    "H",
    "D",
    "HE",
    ...
    for column in df:
        if jon:
            if column in jonlist:
                ...
                starts[column]=start
        elif column not in notspecies or column not in elementList:
...
def JandCratio(species,vel=5,dens=1E3, mylist=False,save=False,longrun=False):
    ratio={}
    values=[]
    region="mid"
    for crir in allcrir:
        ratio[crir]={}
        for uv in alluv:
            output_file="../output/shocks_output/{}shock{}-{}-{}-{}.dat".format("C",dens,vel,crir,uv)
            df=uclchem.analysis.read_output_file(output_file=output_file)
            Cenhs,Clong,Cdests,maxs,starts,avgs1,avgs2,maxT=enhanced(df,dens,vel,shock="C",mylist=mylist)

            output_file="../output/shocks_output/{}shock{}-{}-{}-{}.dat".format("J",dens,vel,crir,uv)
            df=uclchem.analysis.read_output_file(output_file=output_file)
            ...
            if longrun:
                ...
                Cenhs=Clong
                region="post"
            ratio[crir][uv]=Jenhs[species]/Cenhs[species]
    ...
    for crir in allcrir:
        for uv in alluv:
            plt.scatter(np.log10(crir),np.log10(uv),c=ratio[crir][uv],
    ...
    if mylist:
        ...
        name="tomas"
    if save:
        plt.savefig("../output/figures/{}{}criruv_Enh_{}.png".format(name,region,species))
    plt.show()
    plt.close()
