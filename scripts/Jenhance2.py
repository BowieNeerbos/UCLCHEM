...
def Jtable(species,shock="J",mylist=True,save=False,longrun=False):
    ...
    region="mid"
    for dens in alldensity:
        ratio[dens],Jfull[dens],Cfull[dens]={},{},{}
        fullavgJ[dens],fullavgC[dens]={},{}
        for vel in allv:
            output_file="../output/shocks_output/{}shock{}-{}-1-1.dat".format("C",dens,vel)
            df=uclchem.analysis.read_output_file(output_file=output_file)
            Cenhs,Clong,Cdests,Cmaxs,starts,avgs1,avgs2,maxT,Cavgsfull=enhanced(df,dens,vel,shock="C",mylist=mylist,species=species)
            ...
            fullavgC[dens][vel]=Cavgsfull[species]
            fullavgJ[dens][vel]=Javgsfull[species]#gives the avg abundance achieved in j shock
            Cfull[dens][vel]=Cenhs[species]
            Jfull[dens][vel]=Jenhs[species]
    ...
    for dens in alldensity:
        for vel in allv:
            ax.scatter(vel,np.log10(dens),c=ratio[dens][vel],
                        norm=clr.LogNorm(vmin=clrRange**-1,vmax=clrRange),
                        cmap="seismic", marker='s',s=1e3)
            #ax.annotate((np.log10(fullavg[dens][vel])).round(2),(vel-0.25,np.log10(dens)),c="gold")
    f.colorbar(cm.ScalarMappable(norm=clr.LogNorm(vmin=clrRange**-1,vmax=clrRange), cmap="seismic")).set_label("Enhancement factor")
    ax.set(title="J/C enhancement factor for {} - {}-shock".format(species, region))
    ax.set(ylabel=r"$log(\rho_{pre})\; (cm^{-3})$",xlabel= "shock velocity (km/s)")

    ax=f.add_subplot(3,1,2)
    clrRange=10**max(abs(np.log10(min(values))),abs(np.log10(max(values))))
    for dens in alldensity:
        for vel in allv:
            ax.scatter(vel,np.log10(dens),c=Jfull[dens][vel],
                        norm=clr.LogNorm(vmin=clrRange**-1,vmax=clrRange),
                        cmap="seismic", marker='s',s=1e3)
            ax.annotate((np.log10(fullavgJ[dens][vel])).round(2),(vel-0.25,np.log10(dens)),
                        bbox=dict(fc="white",pad=0,alpha=0.5))
    f.colorbar(cm.ScalarMappable(norm=clr.LogNorm(vmin=clrRange**-1,vmax=clrRange), cmap="seismic")).set_label("Enhancement factor")
    ax.set(title="J shock enhancement")
    ax.set(ylabel=r"$log(\rho_{pre})\; (cm^{-3})$",xlabel= "shock velocity (km/s)")
    ax=f.add_subplot(3,1,3)
    for dens in alldensity:
        for vel in allv:
            ax.scatter(vel,np.log10(dens),c=Cfull[dens][vel],
                        norm=clr.LogNorm(vmin=clrRange**-1,vmax=clrRange),
                        cmap="seismic", marker='s',s=1e3)
            ax.annotate((np.log10(fullavgC[dens][vel])).round(2),(vel-0.25,np.log10(dens)),
                        bbox=dict(fc="white",pad=0,alpha=0.5))
    f.colorbar(cm.ScalarMappable(norm=clr.LogNorm(vmin=clrRange**-1,vmax=clrRange), cmap="seismic")).set_label("Enhancement factor")
    ax.set(title="C shock enhancement")
    ax.set(ylabel=r"$log(\rho_{pre})\; (cm^{-3})$",xlabel= "shock velocity (km/s)")
    ...
    if save:
        ...
        plt.savefig("../output/full_enhs/{1}Enh_{2}.png".format(shock,region,species))
    plt.show()
    plt.close()
...
for molecule in df:
    if molecule not in elementList and molecule not in notspecies:
        if not "@" in molecule and not "#" in molecule and not "+" in molecule:
            if not os.path.exists("../output/full_enhs/postEnh_{}.png".format(molecule)):
                Jtable(molecule,mylist=True,longrun=True,save=True)
